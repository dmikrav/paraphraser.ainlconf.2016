...
api = 'AIzaSyCCtpREFeg_1mWkv7AeroI8kFcWmlyeWR0'
...
if __name__ == "__main__":
    ...
    for x in range(n):
      ...
      for i in range(2):
        print
        print pair[x][i]
        sss = re.sub("[^абвгдеёжзийклмнопрстуфхцчшщъыьэюяАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯa-zA-Z .,:%;@0-9-]", "", pair[x][i].encode('utf8'))
        print sss
        print
        res = translator.translate(sss)
        print res
        listt[i] = res[0][u'translatedText'].encode('utf8')
      dict_res[x]["translations"] = {}
      dict_res[x]["translations"]["google"] = {} 
      dict_res[x]["translations"]["google"]['pair'] = listt
    f = open("test_res.json", "w+")
    f.write(json.dumps(dict_res, sort_keys=True, indent=4, separators=(',', ': '), ensure_ascii=False))
    f.close()
